...
const ACTION_CONFIG: Record<string, { icon: any; color: string; label: string; bg: string }> = {
  created:     { icon: Check,         color: 'text-emerald-400', label: 'Compromisso agendado!',   bg: 'bg-emerald-950' },
...
const EXAMPLES = [
  ...
  '"Excluir o almoço de sexta"',
  '"Remarcar a consulta para quarta às 15h"',
  '"Mudar o horário da reunião para as 11h"',
  '"Cancelar o dentista de amanhã"',
  '"Adicionar observação na consulta: trazer exames"',
  '"Marcar como feito o lembrete de hoje"',
]
...
export default function PfAppointmentModal({ open, onClose, onSaved, initial }: Props) {
  ...
  const executeVoice = async (text: string, forceId?: string) => {
    setStep('saving')
    const res = await fetch('/api/pf/appointments/voice', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-gray-900 w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl border border-gray-800 shadow-2xl p-6 flex flex-col items-center gap-4">
        ...
        <div className="flex gap-2 w-full">
          ...
          <button onClick={() => setStep('form')} className="flex-1 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 transition-colors">
            Preencher manual
          </button>
        </div>
      </div>
        ...
        <div className="p-6 flex flex-col items-center">
          <button onClick={listening ? () => recogRef.current?.stop() : startVoice}
            className={`w-24 h-24 rounded-full flex items-center justify-center mb-5 transition-all shadow-xl ${
              listening ? 'bg-emerald-600 animate-pulse shadow-emerald-500/30'
