...
import { Mic, MicOff, X, Check, ChevronRight, Loader2, Calendar } from 'lucide-react'
...
export default function FinAppointmentModal({ open, onClose, onSaved, initial }: Props) {
  ...
  const startVoice = () => {
    ...
    rec.onresult = async (e: any) => {
      ...
      const saved = await fetch('/api/fin/appointments/voice', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (saved.ok) {
        ...
        // Falha no parse — abre formulário pré-preenchido
        const p = saved.parsed ?? {}
        setForm({
          title: p.title ?? '',
          date: p.date ?? today(),
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm">
        <div className="bg-gray-900 w-full sm:max-w-sm sm:rounded-2xl rounded-t-2xl border border-gray-800 shadow-2xl p-8 flex flex-col items-center gap-4">
          <div className="w-20 h-20 rounded-full flex items-center justify-center bg-blue-950">
            <Check size={40} className="text-blue-400" />
          </div>
          <div className="text-center">
            <p className="text-white font-bold text-lg">Compromisso agendado!</p>
            <p className="text-blue-300 font-semibold mt-1">{toastMsg.title}</p>
            ...
            {listening ? (
              ...
              <div className="text-center">
                ...
                <div className="space-y-1 text-center">
                  <p className="text-xs text-gray-500">"Reunião com fornecedor amanhã às 14h"</p>
                  <p className="text-xs text-gray-500">"Ligação para o banco sexta às 10h"</p>
                  <p className="text-xs text-gray-500">"Prazo de entrega dia 28"</p>
                  <p className="text-xs text-gray-500">"Visita ao cliente na próxima terça"</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="bg-gray-900 w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl border border-gray-800 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-3">
          <div>
            ...
            <input type="text" placeholder="Ex: Reunião com fornecedor, Ligação banco..."
              value={form.title ?? ''}
