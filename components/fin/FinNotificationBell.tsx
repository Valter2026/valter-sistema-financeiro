...
export default function FinNotificationBell() {
  ...
  const load = async () => {
    const data = await fetch('/api/fin/notifications').then(r => r.json())
  ...
  const markAll = async () => {
    setLoading(true)
    await fetch('/api/fin/notifications', { method: 'PUT', body: JSON.stringify({ all: true }), headers: { 'content-type': 'application/json' } })
    await load()
    setLoading(false)
  }

  const markOne = async (id: string) => {
    await fetch('/api/fin/notifications', { method: 'PUT', body: JSON.stringify({ id }), headers: { 'content-type': 'application/json' } })
    setNotifs(prev => prev.map(n => n.id === id ? { ...n, read: true } : n))
  }

  const clearRead = async () => {
    await fetch('/api/fin/notifications', { method: 'DELETE' })
    await load()
  }
        ...
        <div className="absolute right-0 top-10 w-80 bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-white">Consultor Empresarial</span>
              {unread > 0 && (
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-gray-800">
            {notifs.length === 0 ? (
              <div className="px-4 py-6 text-center text-gray-500 text-sm">
                ...
                <span className="text-xs">Lance uma transação para receber análises.</span>
              </div>
            ) : (
              notifs.map(n => (
                <div
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {n.action_text && (
                        <p className="text-[10px] text-blue-400 mt-1 font-medium">→ {n.action_text}</p>
                      )}
                      <p className="text-[9px] text-gray-600 mt-1">
                        {new Date(n.created_at).toLocaleString('pt-BR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                      </p>
                    </div>
                    {!n.read && <span className="w-1.5 h-1.5 rounded-full bg-blue-400 flex-shrink-0 mt-1.5" />}
                  </div>
                </div>
              ))
            )}
          ...
          <div className="px-4 py-2.5 border-t border-gray-800">
            <Link href="/financas/consultor" onClick={() => setOpen(false)}
              className="flex items-center justify-center gap-1.5 text-xs text-blue-400 hover:text-blue-300 transition-colors font-medium">
