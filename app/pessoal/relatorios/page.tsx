  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-white">Relatórios</h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex gap-1 bg-gray-800 border border-gray-700 rounded-xl p-1">
            <button onClick={() => setRegime('caixa')}
              className={`px-2 py-1 rounded-lg text-xs font-semibold transition-colors ${regime === 'caixa' ? 'bg-emerald-600 text-white' : 'text-gray-400 hover:text-white'}`}>
              Caixa
            </button>
            <button onClick={() => setRegime('competencia')}
              className={`px-2 py-1 rounded-lg text-xs font-semibold transition-colors ${regime === 'competencia' ? 'bg-emerald-600 text-white' : 'text-gray-400 hover:text-white'}`}>
              Competência
            </button>
          </div>
          <button onClick={() => exportCSV(txs, mes, ano)}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-gray-800 border border-gray-700 text-gray-400 hover:bg-gray-700 hover:text-white transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        <div className="rounded-xl border-l-4 border-emerald-500 bg-emerald-950 p-3 md:p-5">
          <p className="text-[10px] md:text-xs text-gray-400 uppercase tracking-wide mb-1 truncate">Receitas</p>
          <p className="text-sm md:text-xl font-bold text-white tabular-nums truncate">{formatCurrency(totalReceitas)}</p>
          <p className="text-[10px] text-emerald-400 mt-1 flex items-center gap-1"><TrendingUp size={9} /> {MONTHS[mes-1]}</p>
        </div>
        <div className="rounded-xl border-l-4 border-red-500 bg-red-950 p-3 md:p-5">
          <p className="text-[10px] md:text-xs text-gray-400 uppercase tracking-wide mb-1 truncate">Gastos</p>
          <p className="text-sm md:text-xl font-bold text-white tabular-nums truncate">{formatCurrency(totalDespesas)}</p>
          <p className="text-[10px] text-red-400 mt-1 flex items-center gap-1"><TrendingDown size={9} /> {MONTHS[mes-1]}</p>
        </div>
        <div className={`rounded-xl border-l-4 p-3 md:p-5 ${totalReceitas >= totalDespesas ? 'border-blue-500 bg-blue-950' : 'border-red-500 bg-red-950'}`}>
          <p className="text-[10px] md:text-xs text-gray-400 uppercase tracking-wide mb-1 truncate">Resultado</p>
          <p className={`text-sm md:text-xl font-bold tabular-nums truncate ${totalReceitas >= totalDespesas ? 'text-white' : 'text-red-400'}`}>{formatCurrency(totalReceitas - totalDespesas)}</p>
          <p className={`text-[10px] mt-1 ${totalReceitas >= totalDespesas ? 'text-blue-400' : 'text-red-400'}`}>{totalReceitas >= totalDespesas ? '▲ sobrou' : '▼ gastou mais'}</p>
        </div>
      </div>
          ...
          <div className="bg-gray-900 rounded-xl border border-gray-800 p-4 md:p-6 mb-6">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={chart} barSize={18}>
                ...
                <YAxis tick={{ fill: '#6b7280', fontSize: 11 }} tickFormatter={v => `R$${(v/1000).toFixed(0)}k`} width={38} />
                <Tooltip
                  contentStyle={{ background: '#111827', border: '1px solid #374151', borderRadius: 8, fontSize: 12 }}
