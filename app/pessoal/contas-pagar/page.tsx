...
import { Plus, Check, Clock, Pencil, Trash2, AlertTriangle } from 'lucide-react'
import PfTransactionModal from '@/components/pf/PfTransactionModal'

const today = () => new Date().toISOString().split('T')[0]

export default function PfContasPagarPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [t, acc, cat] = await Promise.all([
      fetch('/api/pf/transactions?type=expense').then(r => r.json()),
  ...
  const handlePagar = async (tx: any) => {
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        {([['pendentes','A vencer'],['atrasadas','Em atraso'],['todas','Todas']] as const).map(([k,l]) => (
          <button key={k} onClick={() => setAba(k)}
            className={`px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${
              aba === k ? 'bg-emerald-600 text-white' : 'bg-gray-800 border border-gray-700 text-gray-400 hover:bg-gray-700 hover:text-white'
            }`}>{l}</button>
        ))}
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {lista.map((tx, i) => {
                ...
                return (
                  <tr key={tx.id ?? i} className={`border-b border-gray-800 hover:bg-gray-800 transition-colors ${atrasada ? 'bg-red-950/10' : ''}`}>
                    ...
                    <td className="px-5 py-3.5">
                      {atrasada ? (
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-red-950 text-red-400 border border-red-800">
                          <AlertTriangle size={9} /> {dias}d em atraso
                        </span>
                      ) : (
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex gap-1">
                        <button onClick={() => handlePagar(tx)} title="Marcar como pago"
            ...
            <tfoot>
              <tr className="border-t-2 border-gray-700 bg-gray-800/50">
                <td colSpan={3} className="px-5 py-3 font-bold text-gray-300">Total</td>
