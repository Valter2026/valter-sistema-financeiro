...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabase
    .from('pf_budgets')
    .upsert([{ category_id: body.category_id, month: body.month, year: body.year, amount: body.amount, user_id: user.id }], { onConflict: 'category_id,month,year' })
    .select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await supabase.from('pf_budgets').delete().eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
