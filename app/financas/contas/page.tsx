...
export default function ContasPage() {
  ...
  const [form,     setForm]     = useState<any>(null) // null = fechado, {} = novo, {...} = editar
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="grid grid-cols-3 gap-4">{[...Array(6)].map((_,i) => <div key={i} className="h-32 bg-white rounded-2xl animate-pulse border border-gray-100" />)}</div>
      ) : accounts.length === 0 ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          {accounts.map(acc => {
            ...
            const Icon = cfg.icon
            return (
              <div key={acc.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:border-blue-200 transition-colors">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 rounded-xl" style={{ background: acc.color + '20' }}>
                ...
                <p className="text-xl font-bold" style={{ color: acc.color }}>
                  {formatCurrency(Number(acc.opening_balance ?? 0))}
                </p>
                {acc.bank && <p className="text-xs text-gray-400 mt-2">{acc.bank}</p>}
              </div>
            )
          })}
        </div>
      ...
      {form !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h3 className="text-lg font-bold text-gray-900 mb-5">{form.id ? 'Editar' : 'Nova'} Conta</h3>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Nome da Conta</label>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Banco / Instituição</label>
                <input type="text" value={form.bank ?? ''} onChange={e => setForm({...form, bank: e.target.value})}
                  placeholder="Ex: Itaú, Bradesco, Nubank..."
                  className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Saldo Inicial (R$)</label>
