...
import FinAppointmentModal from '@/components/fin/FinAppointmentModal'
...
const TYPE_COLORS: Record<string, string> = {
  meeting:     'text-blue-400 bg-blue-950',
  ...
  appointment: 'text-emerald-400 bg-emerald-950',
}
...
export default function FinAgendaPage() {
  ...
  const load = useCallback(async () => {
    ...
    const data = await fetch(`/api/fin/appointments?start=${isoDate(firstDay)}&end=${isoDate(lastDay)}`)
      .then(r => r.json()).catch(() => [])
  ...
  // Upcoming appointments (next 7 days, not done/cancelled)
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">Financeiro Empresarial</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setEditing(null); setModalOpen(true) }}
            className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-xl text-sm font-bold transition-colors">
            <Plus size={15} />
            ...
            return (
              <button key={idx} onClick={() => setSelectedDay(isSelected ? null : key)}
                className={`aspect-square border-b border-r border-gray-800/50 flex flex-col items-center justify-start pt-1.5 gap-0.5 transition-colors ${
                  isSelected ? 'bg-blue-950' : isToday ? 'bg-gray-800' : 'hover:bg-gray-800/50'
                }`}>
                <span className={`text-xs font-semibold leading-none ${
                  isToday ? 'text-blue-400' : isSelected ? 'text-blue-300' : 'text-gray-400'
                }`}>{day}</span>
                <div className="flex gap-0.5 flex-wrap justify-center px-0.5">
                  {appts.slice(0, 3).map((a, i) => (
                    <div key={i} className={`w-1.5 h-1.5 rounded-full ${
                      ...
                      a.type === 'meeting' ? 'bg-blue-500' :
                      a.type === 'call' ? 'bg-sky-500' :
                      a.type === 'reminder' ? 'bg-yellow-500' : 'bg-violet-500'
                    }`} />
                  ))}
                </div>
              </button>
