...
const TYPE_CONFIG = {
  ...
  tip:     { label:'Análise',  bg:'bg-gray-900',    border:'border-blue-800',    text:'text-blue-400',    icon: Lightbulb,     iconColor:'text-blue-400'    },
...
function AudioCard({
  ...
  endpoint: string; voice: 'feminina' | 'masculina'; pitch: number
}) {
  ...
  const togglePlay = () => {
    ...
    utter.lang   = 'pt-BR'; utter.rate = 1.0; utter.pitch = pitch
    ...
    utter.onend = () => setPlaying(false); utter.onerror = () => setPlaying(false)
    setPlaying(true); synthRef.current.speak(utter)
  }
        ...
        <div className="flex items-center gap-2">
          ...
          {script && (
            <button onClick={togglePlay}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-colors ${playing ? 'bg-blue-600 text-white' : 'bg-gray-800 border border-gray-700 text-gray-300 hover:bg-gray-700'}`}>
...
export default function FinConsultorPage() {
  const [advices,   setAdvices]   = useState<Advice[]>([])
  const [loading,   setLoading]   = useState(true)
  const [speaking,  setSpeaking]  = useState<number | null>(null)
  const [voice,     setVoice]     = useState<'feminina'|'masculina'>('masculina')
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') synthRef.current = window.speechSynthesis
    fetch('/api/fin/advisor/schedule').then(r => r.json()).then(d => {
  ...
  const loadCache = useCallback(async () => {
    ...
    if (Array.isArray(res.advices) && res.advices.length > 0) {
      setAdvices(res.advices); setGenAt(res.generated_at ?? '')
    }
    setLoading(false)
  }, [])
  ...
  const load = useCallback(async (q = '') => {
    ...
    if (q) {
      const res = await fetch(`/api/fin/advisor?q=${encodeURIComponent(q)}`).then(r => r.json())
      if (res.error) setError(res.error)
      else { setAdvices(res.advices ?? []); setGenAt(res.generatedAt ?? '') }
    } else {
      const res = await fetch('/api/fin/advisor/refresh', { method: 'POST' }).then(r => r.json())
      if (res.error) setError(res.error)
  ...
  const speak = (text: string, idx: number) => {
    ...
    if (sel) utter.voice = sel
    utter.onend = () => setSpeaking(null); utter.onerror = () => setSpeaking(null)
    setSpeaking(idx); synthRef.current.speak(utter)
  }
    ...
    <div>
          ...
          <div className="flex gap-1 bg-gray-800 border border-gray-700 rounded-xl p-1">
            {(['feminina','masculina'] as const).map(v => (
              <button key={v} onClick={() => setVoice(v)}
                className={`px-3 py-1 rounded-lg text-xs font-semibold transition-colors ${voice===v ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}>
                {VOICE_LABEL[v]}
              </button>
            ))}
          </div>
          <div className="flex gap-2">
            {advices.length > 0 && (
              <button onClick={speakAll}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-colors ${speaking===-1 ? 'bg-blue-600 text-white' : 'bg-gray-800 border border-gray-700 text-gray-400 hover:bg-gray-700'}`}>
      ...
      {receiveAudio && (
        <div className="mb-6 space-y-3">
          ...
          <AudioCard
            title="Briefing da Semana"
            icon={Calendar}
            iconColor="text-blue-400"
            endpoint="/api/fin/advisor/weekly"
            voice={voice}
            pitch={pitch}
          />
          <AudioCard
            ...
            endpoint="/api/fin/advisor/monthly"
            voice={voice}
            pitch={pitch}
          />
        </div>
      ...
      {loading && (
        <div className="space-y-3 mb-6">
          <div className="flex items-center gap-3 bg-gray-900 border border-gray-800 rounded-xl p-5">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500/20 to-blue-700/20 flex items-center justify-center animate-pulse">
              <Sparkles size={18} className="text-blue-400" />
            </div>
            <div>
              <p className="text-white font-semibold text-sm">Analisando as finanças da empresa...</p>
              <p className="text-gray-500 text-xs">O consultor está preparando suas orientações empresariais personalizadas</p>
            </div>
          </div>
              ...
              <div key={i} className={`rounded-2xl border p-5 ${cfg.bg} ${cfg.border}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button onClick={() => speak(`${advice.title}. ${advice.message}. Ação recomendada: ${advice.action}`, idx)}
                    className={`p-2 rounded-xl flex-shrink-0 transition-colors ${speaking===idx ? 'bg-blue-600 text-white' : 'text-gray-500 hover:text-gray-300 hover:bg-gray-800'}`}>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
        ...
        <p className="text-xs text-gray-500 mb-4">
          Exemplos: "Como melhorar minha margem?" · "O que fazer com a inadimplência alta?" · "Como reduzir custos fixos?" · "Como calcular o ponto de equilíbrio?"
        </p>
        <div className="flex gap-2">
          <div className="flex-1 relative">
            <input
              type="text"
              placeholder="Digite sua pergunta sobre as finanças da empresa..."
              value={question}
              onChange={e => setQuestion(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleAsk()}
              className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 pr-10 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 transition-colors"
            />
            {question && (
          ...
          <button onClick={listening ? () => recogRef.current?.stop() : startVoiceQuestion}
            className={`px-3 py-3 rounded-xl transition-colors ${listening ? 'bg-blue-600 text-white animate-pulse' : 'bg-gray-800 border border-gray-700 text-gray-400 hover:bg-gray-700'}`}>
            {listening ? <MicOff size={16}/> : <Mic size={16}/>}
          </button>
          <button onClick={handleAsk} disabled={asking || !question.trim()}
            className="flex items-center gap-2 px-5 py-3 rounded-xl bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 disabled:opacity-50 transition-colors">
